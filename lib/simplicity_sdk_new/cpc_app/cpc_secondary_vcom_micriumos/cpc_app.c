/***************************************************************************/ /**
 * @file cpc_app.c
 * @brief Secondary firmware for the CPC application
 * @version 1.0.0
 *******************************************************************************
 * # License
...
static void on_endpoint_error(uint8_t endpoint_id, void *arg)
{
  ...
  if ((endpoint_id == SL_CPC_ENDPOINT_USER_ID_0)
      && (sl_cpc_get_endpoint_state(&user_endpoint_handle_0)
          == SL_CPC_STATE_ERROR_DESTINATION_UNREACHABLE)) {
    // clear connected flag
    OSFlagPost(&endpoint_connected_flags, ENDPOINT_0_CONNECTED, OS_OPT_POST_FLAG_CLR, &err);
    EFM_ASSERT((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE));
  } else if ((endpoint_id == SL_CPC_ENDPOINT_USER_ID_1)
             && (sl_cpc_get_endpoint_state(&user_endpoint_handle_1)
                 == SL_CPC_STATE_ERROR_DESTINATION_UNREACHABLE)) {
    // clear connected flag
    OSFlagPost(&endpoint_connected_flags, ENDPOINT_1_CONNECTED, OS_OPT_POST_FLAG_CLR, &err);
    EFM_ASSERT((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE));
  } else {
    EFM_ASSERT(0);
  }
                               ...
                               sl_status_t status)
{
  ...
  if (endpoint_id == SL_CPC_ENDPOINT_USER_ID_0) {
    ...
    sem = OSSemPost(&tx_complete_semaphore, OS_OPT_POST_ALL, &err);
    EFM_ASSERT((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE));
    EFM_ASSERT(sem != 0);
  } else {
    EFM_ASSERT(0);
...
static void open_endpoint(sl_cpc_endpoint_handle_t *ep, uint8_t endpoint_id)
{
  ...
  // to it
  status = sl_cpc_set_endpoint_option(ep,
  ...
  while (1) {
    ...
    if (flags & ENDPOINT_1_CONNECTED) {
      ...
      if (size > 0) {
        ...
        OSSemPend(&tx_complete_semaphore, 1000, OS_OPT_PEND_BLOCKING, (CPU_TS *)NULL, &err);
        EFM_ASSERT((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE));
      }
    }
  }
...
static void cpc_app_task_connections(void *arg)
{
  ...
  // set connected flag
  OSFlagPost(&endpoint_connected_flags, ENDPOINT_0_CONNECTED, OS_OPT_POST_FLAG_SET, &err);
  EFM_ASSERT((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE));

  open_endpoint(&user_endpoint_handle_1, SL_CPC_ENDPOINT_USER_ID_1);
  // set connected flag
  OSFlagPost(&endpoint_connected_flags, ENDPOINT_1_CONNECTED, OS_OPT_POST_FLAG_SET, &err);
  EFM_ASSERT((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE));
...
void cpc_app_init(void)
{
  ...
  OSSemCreate(&tx_complete_semaphore, "tx complete semaphore", 0u, &err);
  EFM_ASSERT((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE));

  // Create flag group for endpoint disconnect events
  OSFlagCreate(&endpoint_connected_flags, "Endpoint connected flag group", 0u, &err);
  EFM_ASSERT((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE));
