...
static const struct sx_digesttags ba418tags = {
    .cfg = DMATAG_BA418 | DMATAG_CONFIG(0),
    .initialstate = DMATAG_BA418 | DMATAG_DATATYPE(1) | DMATAG_LAST,
 ...
 * (section 5.1 and B.2).
 */
static size_t fips202_pad(unsigned char prefix, unsigned char suffix,
    size_t capacity, size_t msgsz,
...
static void shake256_digest(struct sxhash *c, char *digest)
{
    ...
    padsz = fips202_pad(SHAKE_MODE_PREFIX, SHAKE_MODE_SUFFIX,
        64, c->feedsz, padding);
    ...
    ADD_INDESC_PRIV_RAW(c->dma, OFFSET_EXTRAMEM(c), padsz,
        c->dmatags->data);
...
static void sha3_digest(struct sxhash *c, char *digest)
{
    ...
    padsz = fips202_pad(SHA3_MODE_PREFIX, SHA3_MODE_SUFFIX,
        2 * c->algo->digestsz, c->feedsz, padding);
    /* Use ADD_INDESC_PRIV_RAW instead of ADD_INDESC_PRIV.
     * BA418 hardware cannot work with ADD_INDESC_PRIV as BA418 does not
     * support byte ignore flags. */
    ADD_INDESC_PRIV_RAW(c->dma, OFFSET_EXTRAMEM(c), padsz,
