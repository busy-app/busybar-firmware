...
#include "sl_rail_sdk_simple_cpc_config.h"
#include "sl_cpc.h"
#include "sli_cpc.h"
...
/**************************************************************************//**
 * Simple CPC initialization function
 *****************************************************************************/
...
void sl_rail_sdk_simple_cpc_step(void)
{
  ...
  if (!cpc_opened) {
    if (sl_cpc_get_endpoint_state(&endpoint_handle) == SL_CPC_STATE_FREED) {
      sl_rail_sdk_simple_cpc_init();
    } else {
      sl_rail_sdk_simple_cpc_os_task_proceed();
      return;
    }
  }
  ...
  if (signal_read > 0) {
    ...
    if (status == SL_STATUS_OK) {
      ...
      sl_rail_sdk_simple_cpc_receive_cb(status, data_length, data);
  ...
  if (signal_wr_comp.write_completed > 0) {
    if (cpc_connected) {
      memset(tx_buf, 0, sizeof(tx_buf));
      sl_rail_sdk_simple_cpc_transmit_cb(signal_wr_comp.wr_comp_status);
...
{
  EFM_ASSERT(len <= SL_RAIL_SIMPLE_CPC_TX_BUF_SIZE);
  // Store data in TX buffer until the write completed callback is called
  signal_write = len;
  memcpy((void *)tx_buf, (const void *)data, (size_t)len);
 ...
 *****************************************************************************/
SL_WEAK void sl_rail_sdk_simple_cpc_transmit_cb(sl_status_t status)
{
  (void)status;
}
 ...
 * @note Weak implementation
 *****************************************************************************/
SL_WEAK void sl_rail_sdk_simple_cpc_receive_cb(sl_status_t status,
                                               uint32_t len,
                                               uint8_t *data)
{
  (void)status;
 ...
 * @note Weak implementation.
 *****************************************************************************/
SL_WEAK void sl_rail_sdk_simple_cpc_os_task_proceed(void)
{
