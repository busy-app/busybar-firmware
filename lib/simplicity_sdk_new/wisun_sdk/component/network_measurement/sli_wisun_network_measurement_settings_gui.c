...
static void _phy_fan_form(void *args)
{
  ...
  const uint8_t *profile         = (uint8_t *)args;
  ...
  while (iter != NULL && iter->value_str != NULL) {
    _filter_params.domain = (uint8_t)iter->value;
    tmp_list = app_wisun_get_phy_list(filter_profile);
    if (tmp_list != NULL) {
      domain.domain = (uint8_t)iter->value;
...
static void _phy_domain_form(void *args)
{
  ...
  const phy_domain_list_t *domain = (phy_domain_list_t *)args;
...
static void _set_phy(void *args)
{
  const app_wisun_phy_list_t *phy = (app_wisun_phy_list_t *)args;
...
static sl_status_t _add_domain_to_list(const phy_domain_list_t * const domain)
{
  ...
  if (_domain_list == NULL) {
    _domain_list = sl_malloc(sizeof(phy_domain_list_t));
    if (_domain_list == NULL) {
  ...
  p->next = sl_malloc(sizeof(phy_domain_list_t));
  if (p->next == NULL) {
    return SL_STATUS_FAIL;
  }
