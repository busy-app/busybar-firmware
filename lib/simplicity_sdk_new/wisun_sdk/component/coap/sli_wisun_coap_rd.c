 ...
 *****************************************************************************/
__STATIC_INLINE bool _check_str_cache(const str_cache_t *str);
                                 ...
                                 coap_rd_query_parse_t * parsed)
{
  ...
  if (parsed->resources == NULL) {
    rs = sl_malloc(sizeof(sl_wisun_coap_rhnd_resource_t));
    if (rs == NULL) {
                                    ...
                                    coap_rd_query_parse_t * const dest)
{
  ...
  while (iter != NULL) {
    ...
    if (iter->discoverable
        && _is_pattern_matched(iter, dest)
        && _add_resource(iter, dest) == SL_STATUS_FAIL) {
      return SL_STATUS_FAIL;
    }
    iter = iter->next;
  }
...
static char *_get_uri_query_string(const sl_wisun_coap_packet_t * const packet)
{
  ...
  str = sl_malloc(packet->options_list_ptr->uri_query_len + 1U);
  if (str != NULL) {
...
static sl_status_t _add_dir_to_list(const char *uri_path, coap_rd_query_parse_t * const dest)
{
  ...
  // First dir
  if (dest->dir == NULL) {
    dir = sl_malloc(sizeof(str_cache_t));
    if (dir == NULL) {
      return SL_STATUS_FAIL;
    }
    dest->dir = dir;
                                     ...
                                     const bool add_dir_list)
{
  ...
  str = (char *) sl_malloc(max_str_length + 1U);
    ...
    if (uri_path_len == d_iter->len
      ...
      while (r_iter != NULL) {
        ...
        if (r_iter->discoverable
            ...
            && _add_resource(r_iter, parse) == SL_STATUS_FAIL) {
          return SL_STATUS_FAIL;
        }
        r_iter = r_iter->next;
      }
