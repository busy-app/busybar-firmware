...
void app_memlcd_clear(void)
{
  ...
  if (status != GLIB_OK) {
    app_log_error("Failed to clear display. E[%d]" APP_LOG_NL, (int)status);
  }
}
...
void app_memlcd_update(void)
{
  ...
  if (status != DMD_OK) {
    app_log_error("Failed to update display. E[%d]" APP_LOG_NL, (int)status);
  }
}
...
void app_memlcd_draw_string(display_font_t font, char *format, ...)
{
  ...
  if (status != GLIB_OK) {
    app_log_error("Failed to draw string on display. E[%d]" APP_LOG_NL,
                  (int)status);
  }
}
