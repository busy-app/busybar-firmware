...
static void cleanup_data(uint32_t index, angle_queue_t *queue);
                       ...
                       angle_queue_t *queue)
{
  ...
  if (0 <= cleanup_start) {
    ...
    cleanup_data((uint32_t)cleanup_start, queue);
  }
}
...
{
  for (uint32_t i = 0; i < angle_queue_config.sequence_ids; i++) {
    if ((uint32_t)aoa_sequence_compare(queue->data[i].sequence, sequence) > angle_queue_config.max_sequence_diff) {
