...
sl_zigbee_ezsp_status_t ashDecodeByte(uint8_t byte, uint8_t *out, uint8_t *outLen)
{
  ...
  switch (byte) {
    ...
    case ASH_XON:
    case ASH_XOFF:
      if (!sli_ash_host_get_config(rtsCts)) {
...
{
#ifdef EZSP_HOST
  uint16_t maxTime = sli_ash_host_get_config_or_default(ackTimeMax, ASH_TIME_DATA_MAX);
  uint16_t minTime = sli_ash_host_get_config_or_default(ackTimeMin, ASH_TIME_DATA_MIN);
#else
  uint16_t minTime = sli_ash_ncp_get_config_or_default(ASH_CONFIG_ACK_TIME_MIN, ASH_TIME_DATA_MIN);
  uint16_t maxTime = sli_ash_ncp_get_config_or_default(ASH_CONFIG_ACK_TIME_MAX, ASH_TIME_DATA_MAX);
#endif
...
{
#ifdef EZSP_HOST
  uint16_t nrTime = sli_ash_host_get_config_or_default(nrTime, ASH_NR_TIME);
#else
  uint16_t nrTime = sli_ash_ncp_get_config_or_default(ASH_CONFIG_NR_TIME, ASH_NR_TIME);
#endif
  ashNrTimer =
    (halCommonGetInt16uMillisecondTick() + nrTime) >> ASH_NR_TIMER_BIT;
  if (ashNrTimer == 0) {
    ashNrTimer = 0xFF;
  }
}
