...
sl_status_t sl_imu_init(void)
{
  ...
  status = sl_icm40627_get_device_id(&devid);
  if ( status != SL_STATUS_OK ) {
    goto cleanup;
  }
  ...
  status = sl_icm40627_calibrate_gyro(gyroBiasScaled);
  if ( status != SL_STATUS_OK ) {
    goto cleanup;
  }
...
void sl_imu_configure(float sampleRate)
{
  ...
  /* Set sample rate */
  sensorsSampleRate  = sl_icm40627_set_sample_rate(sampleRate);

  /* Filter bandwidth: 49.30Hz for accel and 92.30Hz for gyro */
  sl_icm40627_accel_set_bandwidth(sl_accelBW_ODR_DIV_2);
  sl_icm40627_gyro_set_bandwidth(sl_gyroBW_ODR_DIV_4);

  /* Accel: 2G full scale */
  sl_icm40627_accel_set_full_scale(sl_accelFS_2g);

  /* Gyro: 62.5 degrees per sec full scale */
  sl_icm40627_gyro_set_full_scale(sl_gyroFS_62_5dps);
  ...
  sl_icm40627_read_interrupt_status(&itStatus);
...
sl_status_t sl_imu_calibrate_gyro(void)
{
  ...
  /* Disable interrupt */
  sl_icm40627_enable_interrupt(false, false);
...
void sl_imu_get_acceleration_raw_data(float avec[3])
{
  ...
  sl_icm40627_accel_read_data(avec);
}
...
void sl_imu_get_gyro_raw_data(float gvec[3])
{
  ...
  sl_icm40627_gyro_read_data(gvec);
}
...
bool sl_imu_is_data_ready(void)
{
  ...
  ready = sl_icm40627_is_data_ready();
