                                                     ...
                                                     uint8_t array_size)
{
  ...
  if (NULL == buffer_ptr) {
    ...
    collect_status = SL_STATUS_NULL_POINTER;
             ...
             || ((uint32_t)buffer_len_max < new_buffer_len)) {
                                                      ...
                                                      uint32_t size)
{
  ...
  } else if (remaining_size <= (uint32_t)required_space) {
    remaining_size = 0;
  } else {
    remaining_size -= (uint32_t)required_space;
  }
                                                ...
                                                uint16_t server_address)
{
  ...
  if (NULL == node) {
    ...
    if (remaining_size < (uint16_t)sizeof(btmesh_conf_dcd_raw_header_t)) {
                                                        ...
                                                        btmesh_conf_task_dcd_iter_cmd_t command)
{
  ...
  remaining_size -= (uint16_t)sizeof(btmesh_conf_dcd_raw_header_t);
  data_idx += (uint16_t)sizeof(btmesh_conf_dcd_raw_header_t);
  ...
  while (0 < remaining_size) {
    ...
    if (remaining_size < (uint16_t)sizeof(btmesh_conf_dcd_raw_element_t)) {
      return SL_STATUS_INVALID_COUNT;
    }
    ...
    total_element_size = (uint16_t)sizeof(btmesh_conf_dcd_raw_element_t)
