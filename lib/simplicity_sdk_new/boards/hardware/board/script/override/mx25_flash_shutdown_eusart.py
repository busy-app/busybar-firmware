...
def compatible(provides, board):
    if board.has_component('spiflash'):
        ...
        if board.get_peripheral_options(Req('eusart'), 'spiflash', {'copi' : 'TX', 'cipo' : 'RX', 'clk' : 'SCLK'}):
            if board.get_peripheral_options(Req('gpio'), 'spiflash', {'cs_n' : None}):
                return 'spiflash'
    return False

    if not hw.provides('device_has_eusart'):
        return False
...
def configure(project, board, instance_name):
    ...
    # MX25 EUSART requirement should not be loaded in the Pin Tool GUI in Studio,
    # as this would effectively block usage of this EUSART for anything else,
