...
def compatible(provides, board):
    icm_40627_component = board.get_components_by_part_number("ICM-40627")

    for sensor_name in sensor_names:
        if board.has_component(sensor_name) and len(icm_40627_component) == 1:
...
def configure(project, board, instance_name):
    req = project.requirement('SL_ICM40627_SPI_EUSART')
    ...
    for sensor_name in sensor_names:
        if board.has_component(sensor_name):
            ...
            int_req = project.requirement('SL_ICM40627_INT')
