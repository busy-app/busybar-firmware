...
#define DECODER_NAME "IMG"

#define FILE_EXTENSION "image"
...
typedef struct {
    ...
    lv_draw_buf_t*
        decompressed; /*Decompressed data could be used directly, thus must also be draw buf*/
...
static lv_result_t decode_indexed_line(
...
static lv_fs_res_t
    fs_read_file_at(lv_fs_file_t* f, uint32_t pos, void* buff, uint32_t btr, uint32_t* br);

static lv_result_t
    decompress_image(lv_image_decoder_dsc_t* dsc, const lv_image_compressed_t* compressed);
...
void lv_img_decoder_init(void) {
    ...
    lv_image_decoder_set_info_cb(decoder, lv_img_decoder_info);
    lv_image_decoder_set_open_cb(decoder, lv_img_decoder_open);
    lv_image_decoder_set_get_area_cb(decoder, lv_img_decoder_get_area);
    lv_image_decoder_set_close_cb(decoder, lv_img_decoder_close);

    decoder->name = DECODER_NAME;
}

lv_result_t lv_img_decoder_info(
    lv_image_decoder_t* decoder,
    ...
    if(src_type == LV_IMAGE_SRC_VARIABLE) {
        ...
        if(lv_strcmp(lv_fs_get_ext(src), FILE_EXTENSION)) return LV_RESULT_INVALID;
        ...
        if(res != LV_FS_RES_OK || rn != sizeof(lv_image_header_t)) {
            LV_LOG_WARN(
                "Read file header failed: %d with len: %" LV_PRIu32 ", expected: %zu",
                res,
                rn,
 ...
 */
lv_result_t lv_img_decoder_open(lv_image_decoder_t* decoder, lv_image_decoder_dsc_t* dsc) {
    LV_UNUSED(decoder);
    ...
    if(dsc->src_type == LV_IMAGE_SRC_FILE) {
        if(lv_strcmp(lv_fs_get_ext(dsc->src), FILE_EXTENSION)) return LV_RESULT_INVALID;
        ...
        if(dsc->header.flags & LV_IMAGE_FLAGS_COMPRESSED) {
            ...
            cf == LV_COLOR_FORMAT_RGB565A8 || cf == LV_COLOR_FORMAT_ARGB8565) {
        ...
        if(dsc->header.flags & LV_IMAGE_FLAGS_COMPRESSED) {
            ...
            if(dsc->args.use_indexed) {
                ...
                res = load_indexed(decoder, dsc);
                use_directly =
                    true; /*If draw unit supports indexed image, it can be used directly.*/
            } else {
                res = decode_indexed(decoder, dsc);
            }
    ...
    if(adjusted != decoded) {
        ...
        free_decoder_data(dsc);
        decoder_data_t* decoder_data = get_decoder_data(dsc);
        decoder_data->decoded =
            adjusted; /*Now this new buffer need to be free'd on decoder close*/
    }
    ...
    lv_cache_entry_t* cache_entry =
        lv_image_decoder_add_to_cache(decoder, &search_key, dsc->decoded, dsc->user_data);
    if(cache_entry == NULL) {
        free_decoder_data(dsc);
        return LV_RESULT_INVALID;
    }
...
void lv_img_decoder_close(lv_image_decoder_t* decoder, lv_image_decoder_dsc_t* dsc) {
    LV_UNUSED(decoder); /*Unused*/
...
lv_result_t lv_img_decoder_get_area(
    lv_image_decoder_t* decoder,
    lv_image_decoder_dsc_t* dsc,
    ...
    bool supported = LV_COLOR_FORMAT_IS_INDEXED(cf) || cf == LV_COLOR_FORMAT_ARGB8888 ||
                     cf == LV_COLOR_FORMAT_XRGB8888 || cf == LV_COLOR_FORMAT_RGB888 ||
    ...
    uint32_t offset = dsc->src_type == LV_IMAGE_SRC_FILE ? sizeof(lv_image_header_t) :
                                                           0; /*Skip the image header*/

    /*We only support read line by line for now*/
    if(decoded_area->y1 == LV_COORD_MIN) {
        /*Indexed image is converted to ARGB888*/
        lv_color_format_t cf_decoded = LV_COLOR_FORMAT_IS_INDEXED(cf) ? LV_COLOR_FORMAT_ARGB8888 :
                                                                        cf;

        decoded = lv_draw_buf_reshape(
            decoder_data->decoded_partial, cf_decoded, w_px, 1, LV_STRIDE_AUTO);
        if(decoded == NULL) {
            if(decoder_data->decoded_partial != NULL) {
                lv_draw_buf_destroy(decoder_data->decoded_partial);
                decoder_data->decoded_partial = NULL;
            }
            decoded = lv_draw_buf_create_ex(
                image_cache_draw_buf_handlers, w_px, 1, cf_decoded, LV_STRIDE_AUTO);
    ...
    if(LV_COLOR_FORMAT_IS_INDEXED(cf)) {
        ...
        if(dsc->src_type == LV_IMAGE_SRC_FILE) {
            buf = lv_malloc(len);
            LV_ASSERT_NULL(buf);
            if(buf == NULL) return LV_RESULT_INVALID;
...
static lv_result_t decode_indexed(lv_image_decoder_t* decoder, lv_image_decoder_dsc_t* dsc) {
    ...
    if(is_compressed) {
        ...
        if(res != LV_FS_RES_OK || rn != palette_len) {
            LV_LOG_WARN(
                "Read palette failed: %d, with len: %" LV_PRIu32 ", expected: %" LV_PRIu32,
                res,
                rn,
                palette_len);
...
#if LV_BIN_DECODER_RAM_LOAD
        draw_buf_indexed = lv_draw_buf_create_ex(
            image_cache_draw_buf_handlers, dsc->header.w, dsc->header.h, cf, dsc->header.stride);
        ...
        if(res != LV_FS_RES_OK || rn != data_len) {
            LV_LOG_WARN(
                "Read indexed image failed: %d, with len: %" LV_PRIu32 ", expected: %" LV_PRIu32,
                res,
                rn,
                data_len);
            goto exit_with_buf;
    ...
    lv_draw_buf_t* decoded = lv_draw_buf_create_ex(
        image_cache_draw_buf_handlers, dsc->header.w, dsc->header.h, LV_COLOR_FORMAT_ARGB8888, 0);
    if(decoded == NULL) {
...
static lv_result_t decode_rgb(lv_image_decoder_t* decoder, lv_image_decoder_dsc_t* dsc) {
    ...
    if(res != LV_FS_RES_OK || rn != len) {
        LV_LOG_WARN(
            "Read rgb file failed: %d, with len: %" LV_PRIu32 ", expected: %" LV_PRIu32,
            res,
            rn,
            len);
        lv_draw_buf_destroy(decoded);
...
static lv_result_t decode_alpha_only(lv_image_decoder_t* decoder, lv_image_decoder_dsc_t* dsc) {
    ...
    if(dsc->header.flags & LV_IMAGE_FLAGS_COMPRESSED) {
        ...
        if(res != LV_FS_RES_OK || rn != file_len) {
            LV_LOG_WARN(
                "Read header failed: %d, with len: %" LV_PRIu32 ", expected: %" LV_PRIu32,
                res,
                rn,
                file_len);
            lv_draw_buf_destroy(decoded);
...
static lv_result_t decode_compressed(lv_image_decoder_t* decoder, lv_image_decoder_dsc_t* dsc) {
    ...
    if(dsc->src_type == LV_IMAGE_SRC_FILE) {
        ...
        if(fs_res != LV_FS_RES_OK || rn != len) {
            LV_LOG_WARN(
                "Read compressed header failed: %d, with len: %" LV_PRIu32
                ", expected: %" LV_PRIu32,
                fs_res,
                rn,
                len);
        ...
        if(compressed->compressed_size != compressed_len) {
            LV_LOG_WARN(
                "Compressed size mismatch: %" LV_PRIu32 " != %" LV_PRIu32,
        ...
        if(fs_res != LV_FS_RES_OK || rn != compressed_len) {
            LV_LOG_WARN(
                "Read compressed file failed: %d, with len: %" LV_PRIu32 ", expected: %" LV_PRIu32,
                fs_res,
                rn,
                compressed_len);
            lv_free(file_buf);
...
static lv_result_t
    ...
    lv_draw_buf_t* decompressed = lv_draw_buf_create_ex(
        image_cache_draw_buf_handlers,
        dsc->header.w,
        dsc->header.h,
        dsc->header.cf,
    ...
    if(decompressed == NULL) {
        LV_LOG_WARN(
            "No memory for decompressed image, input: %" LV_PRIu32 ", output: %" LV_PRIu32,
            input_len,
            out_len);
        return LV_RESULT_INVALID;
    }
    ...
    if(out_len > decompressed->data_size) {
        LV_LOG_ERROR(
            "Decompressed size mismatch: %" LV_PRIu32 " > %" LV_PRIu32,
            out_len,
