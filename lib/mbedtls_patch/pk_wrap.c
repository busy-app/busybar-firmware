...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
int mbedtls_pk_psa_rsa_sign_ext(
    psa_algorithm_t alg,
    ...
    *sig_len = mbedtls_rsa_get_len(rsa_ctx);
    ...
    status = psa_import_key(
        &attributes, buf + MBEDTLS_PK_RSA_PRV_DER_MAX_BYTES - key_len, key_len, &key_id);
    if(status != PSA_SUCCESS) {
        ret = PSA_PK_TO_MBEDTLS_ERR(status);
        goto cleanup;
    }
...
static int rsa_sign_wrap(
    ...
    return mbedtls_pk_psa_rsa_sign_ext(
        psa_alg, pk->pk_ctx, hash, hash_len, sig, sig_size, sig_len);
}
#else /* MBEDTLS_USE_PSA_CRYPTO */
static int rsa_sign_wrap(
    mbedtls_pk_context* pk,
...
static int rsa_decrypt_wrap(
    ...
    status =
        psa_asymmetric_decrypt(key_id, decrypt_alg, input, ilen, NULL, 0, output, osize, olen);
    if(status != PSA_SUCCESS) {
        ret = PSA_PK_RSA_TO_MBEDTLS_ERR(status);
        goto cleanup;
    }
...
static int rsa_check_pair_wrap(
    ...
    return mbedtls_rsa_check_pub_priv(
        (const mbedtls_rsa_context*)pub->pk_ctx, (const mbedtls_rsa_context*)prv->pk_ctx);
}
...
static int ecdsa_verify_psa(
    ...
    ret = mbedtls_ecdsa_der_to_raw(
        curve_bits, p, sig_len, extracted_sig, sizeof(extracted_sig), &converted_sig_len);
    if(ret != 0) {
        goto cleanup;
    }
...
static int ecdsa_opaque_verify_wrap(
    mbedtls_pk_context* pk,
    mbedtls_md_type_t md_alg,
    const unsigned char* hash,
    size_t hash_len,
...
static int ecdsa_verify_wrap(
    ...
    return ecdsa_verify_psa(
        pk->pub_raw, pk->pub_raw_len, curve, curve_bits, hash, hash_len, sig, sig_len);
}
#else /* MBEDTLS_PK_USE_PSA_EC_DATA */
static int ecdsa_verify_wrap(
    mbedtls_pk_context* pk,
    ...
    ret = mbedtls_ecp_point_write_binary(
        &ctx->grp, &ctx->Q, MBEDTLS_ECP_PF_UNCOMPRESSED, &key_len, key, sizeof(key));
    if(ret != 0) {
        return ret;
    }
...
static int ecdsa_verify_wrap(
    ...
    ret = mbedtls_ecdsa_read_signature(
        (mbedtls_ecdsa_context*)pk->pk_ctx, hash, hash_len, sig, sig_len);
...
static int ecdsa_opaque_sign_wrap(
    mbedtls_pk_context* pk,
    mbedtls_md_type_t md_alg,
    const unsigned char* hash,
    size_t hash_len,
...
static int eckey_sign_rs_wrap(
    ...
    MBEDTLS_MPI_CHK(ecdsa_sign_rs_wrap(
        pk, md_alg, hash, hash_len, sig, sig_size, sig_len, f_rng, p_rng, &rs->ecdsa_rs));
...
static int eckey_check_pair_wrap(
    mbedtls_pk_context* pub,
    mbedtls_pk_context* prv,
    int (*f_rng)(void*, unsigned char*, size_t),
    void* p_rng) {
...
static int eckey_check_pair_wrap(
    ...
    return mbedtls_ecp_check_pub_priv(
        (const mbedtls_ecp_keypair*)pub->pk_ctx,
...
#else /* MBEDTLS_PK_USE_PSA_EC_DATA */
static int ecdsa_opaque_check_pair_wrap(
    mbedtls_pk_context* pub,
    mbedtls_pk_context* prv,
    int (*f_rng)(void*, unsigned char*, size_t),
    void* p_rng) {
...
static int rsa_alt_sign_wrap(
    ...
    *sig_len = rsa_alt->key_len_func(rsa_alt->key);
    ...
    return rsa_alt->sign_func(
        rsa_alt->key, f_rng, p_rng, md_alg, (unsigned int)hash_len, hash, sig);
}
...
static int rsa_alt_check_pair(
    ...
    if((ret = rsa_alt_sign_wrap(
            prv, MBEDTLS_MD_NONE, hash, sizeof(hash), sig, sizeof(sig), &sig_len, f_rng, p_rng)) !=
       0) {
        return ret;
    }
...
static int rsa_opaque_sign_wrap(
    mbedtls_pk_context* pk,
    mbedtls_md_type_t md_alg,
    const unsigned char* hash,
    size_t hash_len,
