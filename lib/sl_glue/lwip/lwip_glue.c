...
sys_thread_t
    ...
    furi_thread_set_priority(thread_wrapper->thread, (FuriThreadPriority)prio);
...
err_t sys_thread_finish(sys_thread_t t) {
    ThreadWrapper* thread_wrapper = (ThreadWrapper*)t.thread_handle;
    furi_thread_join(thread_wrapper->thread);
    return ERR_OK;
}
