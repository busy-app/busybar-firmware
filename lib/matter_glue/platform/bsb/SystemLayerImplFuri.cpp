...
/**
 *    @file
 *      This file implements LayerImplFuri. Used by LwIP implementation and OpenThread
 */
...
#include "SystemLayerImplFuri.h"

namespace chip {
namespace System {

LayerImplFuri::LayerImplFuri()
    : mHandlingTimerComplete(false) {
}

CHIP_ERROR LayerImplFuri::Init() {
...
void LayerImplFuri::Shutdown() {
    mLayerState.ResetFromInitialized();
}

CHIP_ERROR LayerImplFuri::StartTimer(
    Clock::Timeout delay,
    ...
    TimerList::Node* timer = mTimerPool.Create(
        *this, SystemClock().GetMonotonicTimestamp() + delay, onComplete, appState);
...
CHIP_ERROR LayerImplFuri::ExtendTimerTo(
    Clock::Timeout delay,
    TimerCompleteCallback onComplete,
    void* appState) {
...
bool LayerImplFuri::IsTimerActive(TimerCompleteCallback onComplete, void* appState) {
    return (mTimerList.GetRemainingTime(onComplete, appState) > Clock::kZero);
}

Clock::Timeout LayerImplFuri::GetRemainingTime(TimerCompleteCallback onComplete, void* appState) {
    return mTimerList.GetRemainingTime(onComplete, appState);
}

void LayerImplFuri::CancelTimer(TimerCompleteCallback onComplete, void* appState) {
    assertChipStackLockedByCurrentThread();
...
CHIP_ERROR LayerImplFuri::ScheduleWork(TimerCompleteCallback onComplete, void* appState) {
    assertChipStackLockedByCurrentThread();
    ...
    TimerList::Node* timer =
        mTimerPool.Create(*this, SystemClock().GetMonotonicTimestamp(), onComplete, appState);
    VerifyOrReturnError(timer != nullptr, CHIP_ERROR_NO_MEMORY);
 ...
 */
CHIP_ERROR LayerImplFuri::StartPlatformTimer(System::Clock::Timeout aDelay) {
 ...
 *
 */
CHIP_ERROR LayerImplFuri::HandlePlatformTimer() {
    VerifyOrReturnError(IsInitialized(), CHIP_ERROR_INCORRECT_STATE);
    ...
    while((timersHandled < CHIP_SYSTEM_CONFIG_NUM_TIMERS) &&
          ((timer = mTimerList.PopIfEarlier(expirationTime)) != nullptr)) {
