...
#define CONVERT_NVM3KEY_TO_KEYMAP_INDEX(nvm3Key) \
    (nvm3Key - SilabsConfig::kConfigKey_KvsFirstKeySlot)
...
CHIP_ERROR KeyValueStoreManagerImpl::Init(void) {
    ...
    CHIP_ERROR error = SilabsConfig::ReadConfigValueBin(
        SilabsConfig::kConfigKey_KvsStringKeyMap,
        reinterpret_cast<uint8_t*>(mKvsKeyMap),
        sizeof(mKvsKeyMap),
        outLen);
    ...
    furi_thread_set_state_callback(
        cleanupThread, [](FuriThread* thread, FuriThreadState state, void* context) {
            if(state == FuriThreadStateStopped) {
                furi_thread_free(thread);
            }
        });

    furi_thread_start(cleanupThread);
...
bool KeyValueStoreManagerImpl::IsValidKvsNvm3Key(uint32_t nvm3Key) const {
    return (
        (SilabsConfig::kConfigKey_KvsFirstKeySlot <= nvm3Key) &&
        (nvm3Key <= SilabsConfig::kConfigKey_KvsLastKeySlot));
}
...
CHIP_ERROR KeyValueStoreManagerImpl::MapKvsKeyToNvm3(
    const char* key,
    uint16_t hash,
    uint32_t& nvm3Key,
    bool isSlotNeeded) const {
    CHIP_ERROR err;
    ...
    for(uint16_t keyIndex = 0; keyIndex < kMaxEntries; keyIndex++) {
        if(mKvsKeyMap[keyIndex] == hash) {
            ...
            CHIP_ERROR err = SilabsConfig::ReadConfigValueBin(
                tempNvm3key, reinterpret_cast<uint8_t*>(strPrefix), length, readCount, 0);
            if(err == CHIP_DEVICE_ERROR_CONFIG_NOT_FOUND) {
                // No steps to be taken, clean up will be done at init
                ChipLogError(
                    DeviceLayer,
                    "Key Hash with no associated NVM3 entry - potential Shadow key detected");
            }
...
void KeyValueStoreManagerImpl::OnScheduledKeyMapSave(System::Layer* systemLayer, void* appState) {
    SilabsConfig::WriteConfigValueBin(
        SilabsConfig::kConfigKey_KvsStringKeyMap,
        reinterpret_cast<const uint8_t*>(mKvsKeyMap),
        sizeof(mKvsKeyMap));
}
...
void KeyValueStoreManagerImpl::ScheduleKeyMapSave(void) {
    ...
    */
    SystemLayer().StartTimer(
        std::chrono::duration_cast<System::Clock::Timeout>(
            System::Clock::Seconds32(SL_KVS_SAVE_DELAY_SECONDS)),
        KeyValueStoreManagerImpl::OnScheduledKeyMapSave,
        NULL);
}
...
CHIP_ERROR KeyValueStoreManagerImpl::_Get(
    ...
    err = SilabsConfig::ReadConfigValueBin(
        nvm3Key,
...
CHIP_ERROR KeyValueStoreManagerImpl::_Put(const char* key, const void* value, size_t value_size) {
    ...
    size_t keyStringLen = strlen(key);
    uint8_t* prefixedData =
        static_cast<uint8_t*>(Platform::MemoryAlloc(keyStringLen + value_size));
...
void KeyValueStoreManagerImpl::ErasePartition(void) {
    ...
    for(uint32_t nvm3Key = SilabsConfig::kMinConfigKey_MatterKvs;
        nvm3Key <= SilabsConfig::kConfigKey_KvsLastKeySlot;
        nvm3Key++) {
...
void KeyValueStoreManagerImpl::KvsMapMigration(void) {
    ...
    CHIP_ERROR err = SilabsConfig::ReadConfigValueBin(
        SilabsConfig::kConfigKey_KvsStringKeyMap,
        reinterpret_cast<uint8_t*>(mKvsStoredKeyString),
    ...
    if(err == CHIP_NO_ERROR) {
        for(uint8_t i = 0; i < oldMaxEntires; i++) {
            if(mKvsStoredKeyString[i][0] != 0) {
                ...
                if(SilabsConfig::ConfigValueExists(nvm3Key, dataLen)) {
                    ...
                    uint8_t* prefixedData =
                        static_cast<uint8_t*>(Platform::MemoryAlloc(keyStringLen + dataLen));
                    VerifyOrDie(prefixedData != nullptr);
                    memcpy(prefixedData, mKvsStoredKeyString[i], keyStringLen);

                    SilabsConfig::ReadConfigValueBin(
                        nvm3Key, prefixedData + keyStringLen, dataLen, readlen);
                    SilabsConfig::WriteConfigValueBin(
                        nvm3Key, prefixedData, keyStringLen + dataLen);
                    mKvsKeyMap[i] =
                        KeyValueStoreMgrImpl().hashKvsKeyString(mKvsStoredKeyString[i]);
                    Platform::MemoryFree(prefixedData);
                }
            }
        }
        ...
        ChipLogError(
            DeviceLayer,
            "Migration failed ! Kvs Key map could not be recovered %" CHIP_ERROR_FORMAT,
            err.Format());
...
int32_t KeyValueStoreManagerImpl::KvsKeyMapCleanup(void* argument) {
    ...
    for(uint16_t key = 0; key < KeyValueStoreManagerImpl::kMaxEntries; key++) {
        ...
        if(mKvsKeyMap[key] != 0 &&
           !SilabsConfig::ConfigValueExists(CONVERT_KEYMAP_INDEX_TO_NVM3KEY(key))) {
