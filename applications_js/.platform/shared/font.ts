// Device font metrics: text width and vertical padding, for any string. Generated by `pnpm fonts:build` from the .font files the firmware runs.
//
// The maps of every font an app draws with are bundled and registered by the
// font-maps plugin (scripts/vite-font-maps.mjs).
...
/** One font as stored in the JSON — see scripts/build-font-maps.mjs. */
type RawFontMap = {
  lineHeight: number
  ascent: number
  descent: number
...
/** Decoded form: absolute codes, searched by binary search. */
type FontMap = {
  lineHeight: number
  ascent: number
  descent: number
...
/** Undo the delta coding. Once per font, on first use. */
...
/**
 * Metrics of a character. Null only outside the font's range; inside it, a code with no glyph of its own reports its neighbour's.
 */
...
/**
 * Vertical metrics in VISIBLE pixel coordinates, from the string's own characters. Subtract `top` to put the pixels on a given row. Zeros for an empty string.
 */
export function textInk(text: string, font: DeviceFont): { top: number; height: number } {
  ...
  for (const ch of text) {
    ...
    if (!m) continue
    const h = m[2]
    // Empty glyphs (space) have no pixels and stay out of the box.
    if (h === 0) continue
    const t = m[1]
    if (t < top) top = t
...
/** Font line height (ascent − descent). Too generous for layout. */
...
/**
 * LAYOUT box: top of the visible pixels to the baseline. Unlike `textInk`, descenders hang outside it, as in ordinary typography.
 */
