...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use plain (unwrapped) key storage; by default keys are wrapped (requires secure boot)",
    )
...
def provision_attestation(
    env: dict, pai_cert: Path, dac_key: Path, dac_cert: Path, wrap: bool = False
) -> None:
...
def main(argv=None):
    ...
    insecure = args.insecure_crypto
    wrap = not insecure

    mode = "insecure" if insecure else "secure"
    certs_source = "production" if args.production_certs else "test"
    print(f"Matter provisioning [{mode}, {certs_source} certs]")

    if wrap:
        info = _get_device_info()
        if info.get("sl_m4_secureboot") != "1":
            raise RuntimeError(
    ...
    if not args.no_attest:
        ...
        with attestation_context as (pai_cert, dac_key, dac_cert):
            provision_attestation(env, pai_cert, dac_key, dac_cert, wrap=wrap)
    ...
    print(
        ...
        f"  passcode={args.passcode} discriminator={args.discriminator}"
    )
