...
class Main(App):
    ...
    def init(self):
        self.parser.add_argument("display", help = "Required display", choices=["front", "back"])
        self.parser.add_argument("--timeout", help = "Ping timeout value in seconds", default=5, type=int, required=False)
    ...
    def prepare_screen(self, display_index):
        if(display_index == 0):
            ...
            self.image_mode = "P"
            self.blk_size = 2
        self.size = self.width, self.height = self.original_width*self.scale, self.original_height*self.scale
    ...
    def on_init(self):
        ...
        self.ws.connect("ws://10.0.4.20/api/v0/screen/ws")
    ...
    def on_render(self):
        if len(self.data) > 0:
            surf = pygame.image.frombuffer(self.data, [self.original_width, self.original_height], self.image_mode)
