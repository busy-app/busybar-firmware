...
def log_cli_command(command: str, response: str, duration: float = None):
    ...
    logger.info(f"CLI command executed: '{command}' -> {len(response)} chars{duration_str}")
...
def log_web_request(method: str, url: str, status_code: int, duration: float = None):
    ...
    getattr(logger, level)(f"Web request: {method} {url} -> {status_code}{duration_str}")
...
def log_assertion(description: str, passed: bool, expected=None, actual=None):
    ...
    getattr(logger, level)(f"Assertion {status}: {description}{expected_str}{actual_str}")
