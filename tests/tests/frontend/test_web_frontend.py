...
class TestWebFrontendBasic:
    ...
    def test_web_frontend_page_loads(self, web_session, web_base_url):
        ...
        with allure.step("Verify page structure for SPA"):
            ...
            nuxt_script = soup.find(
                "script",
                string=lambda text: text and "__NUXT__" in text if text else False,
            )
            assert (
                nuxt_script is not None
            ), "Page should contain Nuxt configuration script"

        allure.attach(
            response.text[:1000], "Page Content Preview", allure.attachment_type.TEXT
        )
    ...
    def test_web_frontend_basic_elements(self, web_session, web_base_url):
        ...
        with allure.step("Check for JavaScript modules"):
            ...
            assert (
                len(module_scripts) > 0
            ), "Page should have JavaScript modules for rendering"
    ...
    def test_web_frontend_dark_theme_support(self, web_session, web_base_url):
        ...
        with allure.step("Check for color mode script"):
            ...
            has_color_mode_script = (
                "nuxt_color_mode" in page_content
                or "__nuxt_color_mode__" in page_content
            )
        ...
        with allure.step("Check for theme-related terms in code"):
            ...
            has_theme_infrastructure = (
                "dark" in page_content
                and "light" in page_content
                and (
            ...
            assert (
                has_theme_infrastructure
            ), "Page should contain theme switching infrastructure"
    ...
    def test_web_frontend_response_headers(self, web_session, web_base_url):
        ...
        with allure.step("Check for charset encoding"):
            ...
            charset_specified = (
                ...
                or "utf-8" in response.text.lower()
                or "charset=utf-8" in response.text.lower()
            )
    ...
    def test_web_frontend_viewport_meta(self, web_session, web_base_url):
        ...
        with allure.step("Check for viewport meta tag"):
            ...
            ), f"Viewport should include width=device-width, got: {content}"
    ...
    def test_web_frontend_bluetooth_elements(self, web_session, web_base_url):
        ...
        with allure.step("Check for Bluetooth related infrastructure"):
            ...
            assert (
                has_app_framework
            ), "Page should have SPA framework for Bluetooth components"

            # Check for prefetch links that might include component resources
            prefetch_links = soup.find_all("link", {"rel": "prefetch"})
            assert (
                len(prefetch_links) > 0
            ), "Page should have prefetch links for dynamic components"
...
class TestWebFrontendStatus:
    ...
    @pytest.mark.frontend
    @pytest.mark.skip(
        "TODO: fix, need to await for statuses to load or not do it via request"
    )
    def test_web_frontend_usb_status(self, web_session, web_base_url):
        ...
        with allure.step("Check for USB/connection status indicators"):
            ...
            has_connection_status = (
                "connected" in page_text or "connection" in page_text
            )
            ...
            has_device_indicators = (
                "busy bar" in page_text
                or "busybar" in page_text
                or "device" in page_text
            )
            ...
            assert (
                has_connection_status or has_usb_in_html or has_device_indicators
            ), f"Page should show USB/connection status. Connection status in text: {has_connection_status}, USB in HTML: {has_usb_in_html}, Device indicators: {has_device_indicators}"
    ...
    def test_web_frontend_screen_streaming_elements(self, web_session, web_base_url):
        ...
        with allure.step("Check for canvas or streaming elements"):
            ...
            streaming_container = soup.find(
                "div",
                class_=lambda x: x and "screen-stream-container" in x if x else False,
            )
            has_streaming_container = streaming_container is not None

            has_streaming_elements = (
                has_canvas or has_video or has_img or has_streaming_container
            )
            assert (
                has_streaming_elements
            ), f"Page should have elements for screen display. Canvas: {has_canvas}, Video: {has_video}, Images: {has_img}, Container: {has_streaming_container}"
    ...
    def test_web_frontend_device_image(self, web_session, web_base_url):
        ...
        with allure.step("Check for device-related images"):
            ...
            device_images = [
                ...
                for img in images
                if any(
                    keyword in str(img.get("src", "")).lower()
                    or keyword in str(img.get("alt", "")).lower()
                    for keyword in [
                        "device",
                        "busy",
                        "bar",
            ...
            device_container = soup.find(
                "div", class_=lambda x: x and "device-image" in x if x else False
            )
            has_device_container = device_container is not None

            assert (
                len(device_images) > 0 or has_device_container
            ), f"Page should contain device-related images. Found {len(device_images)} device images, device container: {has_device_container}"
...
class TestWebFrontendValidation:
    ...
    def test_web_frontend_form_elements(self, web_session, web_base_url):
        ...
        with allure.step("Check for interactive form elements"):
            ...
            interactive_roles = soup.find_all(
                attrs={
                    "role": lambda x: x in ["button", "switch", "link"] if x else False
                }
            )
            ...
            has_interactive_elements = (
                ...
                or len(forms) > 0
                or len(links) > 0
                or len(interactive_roles) > 0
            )
            assert (
                has_interactive_elements
            ), f"Page should have interactive form elements. Buttons: {len(buttons)}, Inputs: {len(inputs)}, Forms: {len(forms)}, Links: {len(links)}, Interactive roles: {len(interactive_roles)}"
    ...
    def test_web_frontend_accessibility_basics(self, web_session, web_base_url):
        ...
        with allure.step("Check for alt attributes on images"):
            ...
            if images:  # Only check if images exist
                ...
                assert (
                    alt_coverage >= 0.5
                ), f"At least 50% of images should have alt attributes, got {alt_coverage * 100:.1f}%"
    ...
    def test_web_frontend_interactive_elements(self, web_session, web_base_url):
        ...
        with allure.step("Check for clickable elements"):
            ...
            cursor_pointer_elements = soup.find_all(
                attrs={
                    "class": lambda x: x and "cursor-pointer" in " ".join(x)
                    if x
                    else False
                }
            )
            ...
            total_interactive = (
                ...
                + len(links)
                + len(clickable_divs)
                + len(cursor_pointer_elements)
                + len(interactive_roles)
            )
            assert (
                total_interactive > 0
            ), f"Page should have interactive elements. Buttons: {len(buttons)}, Links: {len(links)}, Onclick divs: {len(clickable_divs)}, Cursor-pointer: {len(cursor_pointer_elements)}, Interactive roles: {len(interactive_roles)}"
        ...
        with allure.step("Check button accessibility"):
            ...
            if buttons:
                buttons_with_text = [
                    btn
                    for btn in buttons
                    if btn.get_text(strip=True) or btn.get("aria-label")
                ]
                accessibility_ratio = len(buttons_with_text) / len(buttons)
                assert (
                    accessibility_ratio >= 0.8
                ), f"At least 80% of buttons should have text or aria-label, got {accessibility_ratio * 100:.1f}%"
...
class TestWebFrontendMQTT:
    ...
    @pytest.mark.frontend
    @pytest.mark.skip(
        reason="MQTT testing requires specific setup - draft implementation"
    )
    ...
    @pytest.mark.frontend
    @pytest.mark.skip(
        reason="MQTT command testing requires broker setup - draft implementation"
    )
