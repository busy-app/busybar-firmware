...
@allure.epic("BSB API Testing")
@allure.feature("5. Web Frontend")
@allure.story("API (draft)")
class TestSystemAPI:
    """Test cases for System API endpoints"""

    @allure.testcase("2596", "GET /api/version [Draft]")
    @pytest.mark.api
    ...
    @allure.testcase("2598", "GET /api/status [Draft]")
    @pytest.mark.api
    @pytest.mark.frontend
    ...
    @allure.testcase("2599", "GET /api/status/system [Draft]")
    @pytest.mark.api
    @pytest.mark.frontend
    ...
    @allure.testcase("2600", "GET /api/status/power [Draft]")
    @pytest.mark.api
    @pytest.mark.frontend
                ...
                with allure.step(f"Read test file: {test_file}"):
                    ...
                    if read_response.status_code == 200:
                        ...
                        with allure.step(f"Remove test file: {test_file}"):
                            params = {"path": test_file}
                            remove_response = api_session.delete(f"{web_base_url}/api/storage/remove", params=params, timeout=10)
                            
                            if remove_response.status_code == 200:
                                remove_data = remove_response.json()
                                allure.attach(json.dumps(remove_data, indent=2), "Remove Response", allure.attachment_type.JSON)
    ...
    def test_api_wifi_networks(self, api_session, web_base_url):
        ...
        with allure.step("Verify networks response"):
            ...
            if response.status_code == 200:
                ...
                allure.attach(json.dumps(networks_data, indent=2), "WiFi Networks Response", allure.attachment_type.JSON)
    ...
    def test_api_input_key_post(self, api_session, web_base_url):
        ...
        for key in valid_keys:
            with allure.step(f"Send key event: {key}"):
                ...
                assert response.status_code in [200, 400], f"Expected 200 or 400 for key {key}, got {response.status_code}"
                
                if response.status_code == 200:
                    response_data = response.json()
                    allure.attach(json.dumps(response_data, indent=2), f"Input {key} Response", allure.attachment_type.JSON)
...
class TestUpdateAPI:
    ...
    @allure.testcase("2597", "POST /api/update [Draft]")
    @pytest.mark.api
    @pytest.mark.frontend
    def test_api_update_post(self, api_session, web_base_url, persistent_cli_connection):
        ...
        with allure.step("Get initial device info for version comparison"):
            initial_device_info = persistent_cli_connection.execute_command("device_info", timeout=20.0, slow_command=True)
...
class TestAPIErrorHandling:
    ...
    def test_api_file_upload_size_limit(self, api_session, web_base_url):
        ...
        with allure.step("Test large file upload to storage"):
            params = {"path": "/ext/large_test_file.bin"}
            response = api_session.post(
                f"{web_base_url}/api/storage/write",
                params=params,
                data=large_content,
        ...
        with allure.step("Verify size limit handling"):
            ...
            assert response.status_code in [200, 400, 413, 500], f"Expected 200, 400, 413, or 500, got {response.status_code}"
            
            if response.status_code == 200:
                allure.attach("Large file upload succeeded - no size limit enforced", "Upload Result", allure.attachment_type.TEXT)
                # If upload succeeded, try to clean up the large file
                try:
                    cleanup_response = api_session.delete(f"{web_base_url}/api/storage/remove", params={"path": "/ext/large_test_file.bin"}, timeout=10)
                    if cleanup_response.status_code == 200:
                        allure.attach("Large test file cleaned up successfully", "Cleanup Result", allure.attachment_type.TEXT)
                except:
