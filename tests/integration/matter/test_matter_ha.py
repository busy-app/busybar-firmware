...
def test_timer_with_smart_home_lights_lamp(
    ...
    assert _wait_for(lambda: matter_smart_home_api.get_switch_state().state is False)
...
def test_timer_without_smart_home_keeps_lamp_off(
    ...
    while time.monotonic() < deadline:
        assert matter_smart_home_api.get_switch_state().state is False, (
            "device switch turned on despite trigger_smart_home=False"
        )
        assert ha.get_state(entity) == "off", (
...
@pytest.mark.timeout(120)
@pytest.mark.parametrize("state", [True, False], ids=["on", "off"])
def test_device_switch_visible_in_ha(ha, matter_smart_home_api, commissioned_device, state):
    ...
    assert ok, f"HA entity did not become {expected}; observed (t, ha/device): {samples}"
