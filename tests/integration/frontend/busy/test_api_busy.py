...
class TestBusyProfileAPI:
    ...
    def test_api_busy_profile_get_busy(self, busy_api: BusyAPI):
        ...
        assert isinstance(response.busy_bar_settings.show_time_left, bool), (
            "Expected show_time_left to be bool, got "
            f"{response.busy_bar_settings.show_time_left!r}"
        )
    ...
    def test_api_busy_profile_put_custom(self, busy_api: BusyAPI):
        ...
        test_title = f"show_time_left_off_{test_timestamp_ms}"
        ...
        try:
            ...
            with allure.step("Verify the disabled value round-trips"):
                ...
                assert updated.busy_bar_settings.show_time_left is False, (
                    "Expected show_time_left=False, got "
                    f"{updated.busy_bar_settings.show_time_left!r}"
                )
                assert (
    ...
    @allure.title(
        "PUT /api/busy/profiles/custom keeps show_time_left when the key is omitted"
    )
    @pytest.mark.api
    @pytest.mark.frontend
    def test_api_busy_profile_put_custom_without_show_time_left(
        self, busy_api: BusyAPI
    ):
        """Test that an omitted show_time_left value keeps the stored value."""
        original = busy_api.get_profile("custom")
        test_timestamp_ms = original.profile_timestamp_ms + 1
        title_off = f"show_time_left_off_{test_timestamp_ms}"

        try:
            with allure.step("Set show_time_left to false"):
                off_profile = _profile_payload(
                    original,
                    test_timestamp_ms,
                    title=title_off,
                    busy_bar_settings={
                        **original.busy_bar_settings.model_dump(),
                        "show_time_left": False,
                    },
                )
                response = busy_api.set_profile_raw("custom", off_profile)
                assert (
                    response.status_code == 200
                ), f"Expected 200, got {response.status_code}: {response.text[:200]}"
                _wait_for_profile_title(busy_api, "custom", title_off)

            with allure.step("Set a profile without show_time_left"):
                legacy_settings = original.busy_bar_settings.model_dump()
                legacy_settings.pop("show_time_left")
                legacy_timestamp_ms = test_timestamp_ms + 1
                legacy_title = f"legacy_show_time_left_{legacy_timestamp_ms}"

                legacy_profile = _profile_payload(
                    original,
                    legacy_timestamp_ms,
                    title=legacy_title,
                    busy_bar_settings=legacy_settings,
                )
                response = busy_api.set_profile_raw("custom", legacy_profile)
                assert (
                    response.status_code == 200
                ), f"Expected 200, got {response.status_code}: {response.text[:200]}"

            with allure.step("Verify the stored value survives"):
                _wait_for_profile_title(busy_api, "custom", legacy_title)
                ...
                assert (
                    raw_settings.get("show_time_left") is False
                ), f"Expected show_time_left=False, got {raw_settings!r}"
        finally:
            with allure.step("Restore original profile"):
                current = busy_api.get_profile("custom")
                restore_data = _profile_payload(
