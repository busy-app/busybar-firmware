...
static void this_var_item_list_auto_update_callback(VarItem* item, void* context) {
    UNUSED(item);
    UNUSED(context);
}
...
static void this_scene_on_enter(void* context) {
    ...
    with_gui(instance->gui, {
        ...
        VarItem* front_auto_update_item = var_item_list_add_switch(
            scene->front_list, "Auto-update", this_var_item_list_auto_update_callback, instance);
        var_item_set_value(front_auto_update_item, true);
        ...
        var_item_set_value(back_auto_update_item, true);
    });
}
