...
static void account_scene_not_linked_menu_item_callback(uint32_t index, void* context) {
    account_settings_send_custom_event(context, index);
}

static void account_scene_not_linked_menu_on_enter(void* context) {
    furi_assert(context);

    AccountSettings* instance = context;
    SceneNotLinkedMenu* data =
        scene_manager_get_scene_data(instance->scene_manager, SceneIdNotLinkedMenu);
...
static void account_scene_not_linked_menu_on_exit(void* context) {
    furi_assert(context);
...
static bool account_scene_not_linked_menu_on_event(const SceneManagerEvent* event, void* context) {
    furi_assert(context);
...
const Scene account_scene_not_linked_menu = {
    .enter_callback = account_scene_not_linked_menu_on_enter,
    .exit_callback = account_scene_not_linked_menu_on_exit,
    .event_callback = account_scene_not_linked_menu_on_event,
    .data_size = sizeof(SceneNotLinkedMenu),
};
