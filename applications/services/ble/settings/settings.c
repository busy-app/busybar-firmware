...
bool ble_settings_reset(BleSettings* settings) {
    ...
    bool is_successful = setting_provider_reset(provider, &BLE_SETTINGS_ROOT, settings);
...
bool ble_settings_load(BleSettings* settings) {
    ...
    SettingProvider* provider =
        setting_provider_alloc(BLE_SETTINGS_FILE_PATH, BLE_SETTINGS_VERSION, NULL, 0);
    bool is_successful = setting_provider_load(provider, &BLE_SETTINGS_ROOT, settings);
    setting_provider_free(provider);
...
bool ble_settings_save(const BleSettings* settings) {
    ...
    SettingProvider* provider =
        setting_provider_alloc(BLE_SETTINGS_FILE_PATH, BLE_SETTINGS_VERSION, NULL, 0);
    bool is_successful = setting_provider_save(provider, &BLE_SETTINGS_ROOT, settings);
    setting_provider_free(provider);
