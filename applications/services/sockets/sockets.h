 ...
 *
 * This function is asynchronous (does not block). The event callback will be called
 * with the appropriate parameters when the sending process has been completed.
 *
 ...
 * @param[out] sent_size Pointer to the variable to hold the number of bytes actually sent (may be NULL)
 *
 * @returns SocketStatusOk on success, a SocketStatus error code otherwise
 */
