...
static void app_terminate_background_tasks(JsRunnerApp* app) {
    abort_fetches(&app->fetch);
}

static void app_terminate(JsRunnerApp* app) {
    app->terminate = true;
    app_terminate_background_tasks(app);
}
