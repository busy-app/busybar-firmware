...
} SettingsSceneMatterReset;

static void settings_scene_matter_reset_on_enter(void* context) {
    furi_assert(context);
    SettingsApp* app = context;
    SettingsSceneMatterReset* scene = scene_manager_get_current_scene_data(app->scene_manager);
    ...
    with_gui(app->gui, {
        ...
        /* front */ {
            ...
            label_set_text(scene->front.message, "Restarting device...");
        ...
        /* back */ {
            ...
            label_set_text(scene->back.message, "Restarting device...");
    ...
    furi_delay_ms(RESTART_TIMER_MS);
...
static void settings_scene_matter_reset_on_exit(void* context) {
    furi_assert(context);
    SettingsApp* app = context;
    SettingsSceneMatterReset* scene = scene_manager_get_current_scene_data(app->scene_manager);
...
static bool settings_scene_matter_reset_on_event(const SceneManagerEvent* event, void* context) {
    furi_assert(context);
...
const Scene settings_scene_matter_reset = {
    .enter_callback = settings_scene_matter_reset_on_enter,
    .exit_callback = settings_scene_matter_reset_on_exit,
    .event_callback = settings_scene_matter_reset_on_event,
    .data_size = sizeof(SettingsSceneMatterReset),
};
