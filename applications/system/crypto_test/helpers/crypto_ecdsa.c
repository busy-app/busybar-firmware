...
void crypto_ecdsa_command(PipeSide* pipe, FuriString* args, void* context) {
    ...
    FuriHalCryptoEcdsa* handle = furi_hal_crypto_ecdsa_verify_init(FuriHalCryptoEcdsaModeSha256, &public_key);
    if(furi_hal_crypto_ecdsa_verify(
    ...
    crypto_common_print_buffer_hex(
        "Key =\t\t", private_key.data, private_key.length);
    crypto_common_print_buffer_hex(
        "Wrapped key =\t", private_key_wrap->data, private_key.length);
    ...
    furi_check(status == FuriHalCryptoStatusOk);
    crypto_common_print_buffer_hex(
        "Key =\t\t", private_key_224.data, private_key_224.length);
    crypto_common_print_buffer_hex(
