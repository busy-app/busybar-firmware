...
static void ble_usart_echo_motd(void* context) {
    UNUSED(context);
    printf("\r\n+-----------------------------+\r\n");
...
void ble_usart_echo_command(PipeSide* pipe, FuriString* args, void* context) {
    ...
    CliShell* shell = cli_shell_alloc(ble_usart_echo_motd, NULL, pipe, registry, NULL);
    cli_shell_set_prompt(shell, "ble_usart");

    cli_shell_start(shell);
    BLEUsartEchoApp* app = ble_usart_echo_app_start(shell);
    cli_shell_join(shell);
    ble_usart_echo_app_stop(app);
