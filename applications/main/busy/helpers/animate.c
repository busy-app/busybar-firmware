...
static void animate_pos_x_animation_callback(void* context, int32_t value) {
    furi_assert(context);

    Widget* target = context;
    widget_set_pos_x(target, value);
}
...
void animate_pos_x(Widget* target, int32_t from, int32_t to, uint32_t time_ms) {
    furi_assert(target);
    ...
    lv_anim_set_exec_cb(&anim, animate_pos_x_animation_callback);
