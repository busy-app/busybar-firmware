...
static bool
    ...
    return is_finite_mode && instance->config.is_show_time_left_enabled;
}
...
static bool
    ...
    const bool is_finite_mode = data->timer_mode == BusyTimerModeSimple ||
                                data->timer_mode == BusyTimerModeInterval;
    const bool is_hidden_time = !instance->config.is_show_time_left_enabled;

    return is_finite_mode && (busy_scene_timer_has_label_tweaks(data) || is_hidden_time);
}
...
static const TimerIndicatorPreset*
    ...
    if(timer_state == BusyTimerStateWork) {
        if(data->is_custom_theme) {
            ...
            } else if(!instance->config.is_show_time_left_enabled) {
                // NOTE: With the time left hidden, a finite Work phase uses the
                // full-width indicator that has a progress fill
                ret = &busy_timer_indicator_presets[BusyTimerIndicatorTypeWorkBigProgress];
            } else {
        ...
        // NOTE: Rest only occurs in Interval mode, so there is no Infinite case here
        if(!instance->config.is_show_time_left_enabled) {
            ret = &busy_timer_indicator_presets[BusyTimerIndicatorTypeRestBig];
        } else {
...
static const TimerIndicatorTransition*
    ...
    if(!busy_scene_timer_has_label_tweaks(data)) {
        // NOTE: With the time left hidden, the indicator stays full-width after
        // an Infinite to Simple change, so no shrink transition is played
        if(data->is_mode_transition && instance->config.is_show_time_left_enabled) {
...
static void busy_scene_timer_update_timer_state(BusyApp* instance) {
    ...
    with_gui(instance->gui, {
        ...
        if(timer_label_preset) {
            ...
            // NOTE: With the time left hidden, peeks draw over the full-width
            // indicator, so they need the gradient backdrop to stay legible
            timer_label_enable_background(
                data->timer_label,
                has_label_tweaks || !instance->config.is_show_time_left_enabled);
