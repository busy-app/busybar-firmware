...
static void rle_test(size_t blk_size) {
    ...
    mu_assert_int_eq(true, rle_compress(original, sizeof(original), compressed, sizeof(compressed), blk_size, &compressed_sz));

    uint8_t decompressed[sizeof(original)];
    size_t decompressed_sz = 0;
    mu_assert_int_eq(true, rle_decompress(compressed, compressed_sz, decompressed, sizeof(decompressed), blk_size, &decompressed_sz));
